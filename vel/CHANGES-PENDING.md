...
| A4 | **Pull the RCM lines into ITC Register 2026-27.** Its `Category` column is 100% ITC today — measured 23-09: all 2,109 rows read `ITC`, not one RCM row. The FY 26-27 claims of FY 25-26 RCM belong there and feed Table 13 (and 12C). Source is the RCM Register, column **BK `GSTR 3B Claim month`**, the rows reading **April 2026 — 392 of its 3,631 rows** — brought in the same format as the ITC lines, with `Category` = RCM. **Traps:** (a) BK is mixed text and dates (`April 2026` as text, `Mar-26`, `05 Aug 202` truncated), so match on text as well as date; (b) the A14 rule applies to these rows too — the RCM must be DATED in FY 25-26, not merely labelled; (c) unclaimed RCM comes through the receivable GL reconciliation. | ITC Register 2026-27, RCM Register, ITC Summary (Table 13) | M1, Pawan 23-09 | todo |
...
| 23-09-2026 | A4 | Confirmed and sized from the master: the 26-27 sheet is 2,109 ITC rows with zero RCM; 392 RCM Register rows carry claim month April 2026. |
