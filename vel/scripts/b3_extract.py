...
for f in sorted(os.listdir(B)):
    ...
    for i in range(1,len(d)):
        ...
        if not sec.startswith("3.1.A"): continue
    ...
    rows.append(rec)
    print(f"  {st:22s} {gstin} | 3.1.A rows present: {found}/4")
T=pd.DataFrame(rows); T.to_pickle(os.path.join(SP,"b3.pkl"))
print(f"\nfiles read: {len(T)}")
tot={v:sum(T[f'{m}|{v}'].sum() for m in [c.split('|')[0] for c in T.columns if '|taxable' in c]) for v in ['taxable','igst','cgst','sgst']}
print("FY totals from 3B table 3.1(a):")
