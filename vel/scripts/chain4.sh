...
for s in cascade_fix final_countif_rule t6a1_2b_period fy2627_reco remarks_mirror rcm_gl_rebuild; do
  echo "=== $s START $(date +%H:%M:%S)"
  "$PY" -X utf8 "$s.py" || { echo "=== $s FAILED rc=$?"; echo "=== CHAIN4 ABORTED"; exit 1; }
