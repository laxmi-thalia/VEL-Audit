# One Excel session, one full rebuild, ALL golden checks (master_verify set + fix2 set)
import win32com.client as win32, pythoncom
P = r"C:\Users\pawar\AppData\Local\Temp\claude\c--PROJECTS-accountic\ed6b1fc9-75d0-4eb0-9100-e931702d9fa7\scratchpad\master_snapshot.xlsx"
...
try:
    ...
    g=wb.Worksheets("Step 1 Flags"); ck("gate blocking",float(g.Range("B4").Value),3,0)
    ...
    ck("S2 MoM residual sum",xl.WorksheetFunction.Sum(mm.Range("M4:M1371")),-9.42,0.5)
    ...
    sv=wb.Worksheets("S3 SR vs 3B"); lv=sv.Cells(sv.Rows.Count,2).End(-4162).Row
    ck("S3 SRvs3B diff",sv.Cells(lv,5).Value,51642491.03)
    ...
    for r,v in {2:365858,3:43888,4:20076,5:-5837,6:634499.7,7:46406,8:67548.81,9:-634499.7,10:-67548.81}.items():
        ck("S4 Exc row %d contribution"%r,ex.Cells(r,8).Value,v,0.02)
    h5=wb.Worksheets("S5 HSN Summary"); l5=h5.Cells(h5.Rows.Count,1).End(-4162).Row
    ck("S5 HSN taxable",h5.Cells(l5,7).Value,8486105710.82)
    ck("S5 HSN qty",h5.Cells(l5,6).Value,53395416.32)
    r5=wb.Worksheets("S5 Rate-wise"); lr5=r5.Cells(r5.Rows.Count,1).End(-4162).Row
    ...
    gac=wb.Worksheets("S6 GL Advance Check")
    bad=sum(1 for r in range(4,12) if abs(gac.Cells(r,5).Value or 0)>=1)
    ...
    advr=None
    for r in range(6,20):
        if str(s9.Cells(r,1).Value or "").startswith("Unadjusted advances"): advr=r
        if str(s9.Cells(r,1).Value or "")=="Difference (A)":
            ck("S9 Difference (A) total",s9.Cells(r,21).Value,3771605598.21,1)
    ...
    ck("S9 adv Gujarat",s9.Cells(advr,hdr["Gujarat"]).Value,-51069933.33)
    ck("S9 adv MP",s9.Cells(advr,hdr["Madhya Pradesh"]).Value,81524711.11)
    ck("S9 adv total (=control net)",s9.Cells(advr,21).Value,235365455.56,0.2)
